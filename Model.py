#Import statements - Using Tensorflow
import numpy as np 
import pandas as pd 
import os
...
#Dataframe using Kaggle Dataset
...
#Seperature train, validation test sets
print(df.head(50))
train_df, valid_df = skl.train_test_split(df, test_size = 0.2,)

#image modification through generators
...
#Utilize pretrained VGG16 image model
model = VGG16(include_top=True, weights='imagenet')
transfer_layer = model.get_layer('block5_pool')
vgg_model = Model(inputs=model.input,outputs=transfer_layer.output)

#Establish unchangable layers
for layer in vgg_model.layers[0:17]:
    layer.trainable = False
    
    
#Adding layers to pretrained model + dropout for overfitting
...
#Determine optimizer, loss, performance metric
optimizer = Adam(lr=1e-5)
loss = 'CategoricalCrossentropy'
metrics = ['binary_accuracy']
print(new_model.summary())

#Compile model + establish history, filepath
